...
class YoloObstacleDetector:
    """CPU-only YOLOv8 TorchScript detector with 5 FPS threaded capture loop."""
    ...
    def load(self) -> None:
        """Load model once globally for this process."""
        if torch is None:
    ...
    def get_latest(self) -> List[Detection]:
        """Non-blocking read of latest detections generated by inference thread."""
    ...
    def _estimate_distance(frame_h: int, bbox: Tuple[float, float, float, float]) -> float:
        _, y1, _, y2 = bbox
        box_h = max(1.0, y2 - y1)
        normalized = box_h / frame_h
        return round(max(0.5, 10.0 * (1.0 - normalized)), 2)
    ...
    def _is_moving(
        ...
        box_shift = abs(x1 - px1) + abs(y1 - py1) + abs(x2 - px2) + abs(y2 - py2)
        return diff_motion or box_shift > 20.0
